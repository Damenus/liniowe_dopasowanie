...
public class SequenceComparer {
    ...
    private ArrayList mismatchIndices = new ArrayList();
    ...
    public static void main(String[] args) throws CompoundNotFoundException {
        SequenceComparer sc = new SequenceComparer(new DNASequence("TCAT"), new DNASequence("CCGC", AmbiguityDNACompoundSet.getDNACompoundSet()));
        System.out.println(sc.getDifferenceCount());
        System.out.println(sc.getConsensusSequence());
        sc = new SequenceComparer(new DNASequence("TCAT"), new DNASequence("CCGC", AmbiguityDNACompoundSet.getDNACompoundSet()));
        System.out.println(sc.getDifferenceCount());
        System.out.println(sc.getConsensusSequence());

        //ProteinSequence s1 = getSequenceForId(id1);
        //ProteinSequence s2 = getSequenceForId(id2);
        //SubstitutionMatrix<NucleotideCompound> matrix = new SimpleSubstitutionMatrix<AminoAcidCompound>();

        // najlepsze globalne
        SubstitutionMatrix<NucleotideCompound> matrix = SubstitutionMatrixHelper.getNuc4_4();
        SequencePair<DNASequence, NucleotideCompound> pair = Alignments.getPairwiseAlignment(new DNASequence("ACGT"), new DNASequence("ACC"), Alignments.PairwiseSequenceAlignerType.GLOBAL, new SimpleGapPenalty(), matrix);
        System.out.printf("%s", pair);

        SubstitutionMatrix<NucleotideCompound> matrix2 = SubstitutionMatrixHelper.getNuc4_4();
        SequencePair<DNASequence, NucleotideCompound> pair2 = Alignments.getPairwiseAlignment(new DNASequence("ACCCCCACGTCAAAAAAA"), new DNASequence("ACGT"), Alignments.PairwiseSequenceAlignerType.LOCAL, new SimpleGapPenalty(), matrix2);
        System.out.printf("%n%s vs %s%n%s", pair2.getQuery().getAccession(), pair2.getTarget().getAccession(), pair2);
    }
